...
# Categorical
train.loc[:, train.select_dtypes(include=["object", "category"]).columns] = (
    train.select_dtypes(include=["object", "category"]).fillna("missing").astype(str)
)
# Numerical
train.loc[:, train.select_dtypes(include="number").columns] = (
    train.select_dtypes(include="number").fillna(-9999)
)

# Labelencode because SMOTE cannot handle categorical features 
for c in cat_cols:
    train[c] = LabelEncoder().fit_transform(train[c].astype(str))
...
results.to_csv(project_root / "cat_smote.csv")
